...
begin;
  select app_fn.create_tenant(
    _name => 'Extra Thick'::citext
    ,_identifier => 'extra-thick'::citext
...
begin;
  select app_fn.subscribe_tenant_to_license_pack(
    (select id from app.tenant where identifier = 'extra-thick')
    ,'bandz'
  );
commit;

begin;
  select app_fn.invite_user(id, 'brassmoose@gmail.com', 'admin') from app.tenant where identifier = 'extra-thick';
  -- select app_fn.invite_user(id, 'EMAIL', 'user') from app.tenant where identifier = 'extra-thick';
